...
def _plot_standardized_pearson_residuals_vs_fitted(df : pd.DataFrame
                                                   , model : Any
...
def _plot_standardized_residuals_by_column(df, model, x_col, title=None, return_=False) -> None:
...
def residual_plot(df, model, column=None, width=None, height=None, margin=None) -> None:
    ...
    if column is None:
        ...
        fig = make_subplots(rows=2, cols=2, subplot_titles=("Standardized Pearson Residuals by Dev",
                                                            "Standardized Pearson Residuals by AY",
                                                            "Standardized Pearson Residuals by Cal",
                                                            "Standardized Pearson Residuals vs Fitted Values"))

        dev_plot = _plot_standardized_residuals_by_column(df, model, 'dev', return_=True)
        ay_plot = _plot_standardized_residuals_by_column(df, model, 'ay', return_=True)
        cal_plot = _plot_standardized_residuals_by_column(df, model, 'cal', return_=True)
        fitted_plot = _plot_standardized_pearson_residuals_vs_fitted(df, model, return_=True)
