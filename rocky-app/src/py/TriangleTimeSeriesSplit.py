...
class TriangleTimeSeriesSplit:
    ...
    def GridTweedie(self, alpha=None, p=None, max_iter=None):
        if alpha is not None:
            self.tweedie_grid["alpha"] = alpha

        if p is not None:
            self.tweedie_grid["p"] = p
    ...
    def TuneTweedie(self):
        ...
        parameters = list(
            itertools.product(self.tweedie_grid["alpha"], self.tweedie_grid["p"])
        )
        n_parameters = len(parameters)

        for alpha, p in parameters:
            result_dict[f"alpha_{np.round(alpha, 2)}_p_{np.round(p, 2)}"] = {}

        for train, val in self.GetSplit():
            X_train = self.tri.get_X_base().iloc[train]
            y_train = self.tri.get_y_base()[train]
            X_id_train = self.tri.get_X_id().iloc[train]
            ...
            for alpha, p in tqdm(
                ...
                result_dict[f"alpha_{np.round(alpha, 2)}_p_{np.round(p, 2)}"][
                    ...
                    y_train, model.predict(X_train), power=np.round(p, 2)
                )
                result_dict[f"alpha_{np.round(alpha, 2)}_p_{np.round(p, 2)}"][
                    f"cy_{excl_cal}_d2_test"
                ] = d2_tweedie_score(
                    y_test, model.predict(X_test), power=np.round(p, 2)
                )
            ...
            for c in tweedie_result.columns.tolist():
                ...
                tweedie_result.loc["p", c] = np.round(float(p), 2)
    ...
    def TweedieParetoFront(self, measures=None):
        ...
        measures: list, default=None
            ...
            the Pareto front, and the values are 'min' if we want to minimize that measure
            or 'max' if we want to maximize that measure. For example, if we want to
            minimize the mean squared error and maximize the d2 score, we would pass
            measures={'mse': 'min', 'd2': 'max'}. If None, the default is to minimize
            the mean squared error and minimize the standard deviation of the mean
            squared error. This is the recommended setting, and is meant to balance
        ...
        # Initialize a boolean Series to keep track of whether each model is Pareto optimal
    ...
    def OptimalTweedie(self, measures=None, tie_criterion=None):
        ...
        best_model = TweedieRegressor(alpha=alpha, power=p, link="log")
