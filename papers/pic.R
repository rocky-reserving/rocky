...
PaidIncurredChain <- function(triangleP,triangleI) {
  ...
  # diagonal of triangleP
  diagP <- diag(triangleP[J:1, 1:J])[(J-1):1]

  # log(P_{i,j}/P_{i,j-1}) triangle

  # fP is the log(P_{i,j}/P_{i,j-1}) triangle -- the 'f' stands for 'fraction'
  fP <- matrix(data=NA, nrow=J, ncol=J)
  for (i in 1:J) {
    # first column is log(P_{i,1})
    fP[i, 1] <- log(triangleP[i, 1])
    if (i == J) {
      break
    }
    for (j in 1:(J-i)) {
      # other columns are log(P_{i,j+1}/P_{i,j}) - eg the log of the age-to-age
      # ratio of the cumulative payments
