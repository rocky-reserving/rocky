...
class HalfMegaModel:
  ...
  def prior_ultimate_distributions(self
    ...
    if standalone:
      alpha_loss = pymc.LogNormal.dist(mu=m_l,
                                       sigma=s_l,
                                       shape=self.rpt_loss_tri.tri.shape[0])
    else:
      _ult_loss = pymc.LogNormal('latent_ult_loss',
                                      mu=m_l,
                                      sigma=s_l,
      ...
      # the MegaModel class
      alpha_loss = pymc.Deterministic('alpha_loss', _ult_loss)
  ...
  def chain_ladder_model(self):
    with pymc.Model() as model:
      ...
      beta_rpt_loss, beta_paid_loss = self.prior_development_distributions(standalone=False)

      # prior distributions for the standard deviations
      sigma_rpt_loss, sigma_paid_loss = self.prior_sigma_distributions(standalone=False)
      sigma_rpt = np.array([sigma_rpt_loss.eval()
                            for _ in range(self.n_rows)])[self.tri_mask]
      sigma_paid = np.array([sigma_paid_loss.eval()
                             for _ in range(self.n_rows)])[self.tri_mask]
